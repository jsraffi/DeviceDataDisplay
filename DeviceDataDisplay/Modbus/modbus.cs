...
namespace DeviceDataDisplay.Modbus
{
    public class modbus
    {
        ...
        public bool SendFc16(byte address, ushort start, ushort registers, short[] values,byte[] resbytes= null)
        {
            //Ensure port is open:
            if (sp.IsOpen)
            {
                ...
                if (CheckResponse(response))
                {
                    ...
                    for (int i = 0; i < (response.Length - 5) / 2; i++)
                    {
                      ...
                      /*  
                        ...
                        values[i] <<= 8;
                        values[i] += response[2 * i + 5];
                        values[i] <<= 8;
                        values[i] += response[2 * i + 6];
                        */

                        //values[i] += response[2 * i + 4] <<= 8 | response[2 * i + 3];
                    }
